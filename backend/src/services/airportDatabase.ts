/**
 * Commercial Airport Database & Nearest Airport Resolver — TripSage
 *
 * Authoritative mapping of commercial airports worldwide and in India with GPS coordinates.
 * Prevents non-airport cities (e.g. Medak, Nizamabad, Alwar, Ooty) from generating fake flights
 * and calculates exact nearest commercial airport hubs with distance in kilometers.
 */
...
export interface AirportValidationResult {
  ...
  airport: CommercialAirport | null;
  nearestAirport?: {
    iata: string;
    name: string;
    city: string;
...
// ── Master Database of Commercial Airports ────────────────────────────────────
export const COMMERCIAL_AIRPORTS: CommercialAirport[] = [
...
// ── Known Cities Lacking Commercial Airports & Their Nearest Commercial Airport ──
const NON_AIRPORT_CITIES_MAP: Record<string, { nearestIata: string; distanceKm: number }> = {
  ...
  agra: { nearestIata: 'DEL', distanceKm: 190 }, // Commercial flights to DEL/LKO primary
...
/**
 * Validates whether a given query matches a commercial airport.
 */
export function validateCommercialAirport(query: string | undefined): AirportValidationResult {
  if (!query) {
  ...
  for (const airport of COMMERCIAL_AIRPORTS) {
    if (
      ...
      airport.keywords.some((kw) => kw === mainCity || mainCity.includes(kw))
    ) {
  ...
  // 4. City has no direct commercial airport — resolve nearest default hub (e.g., HYD/DEL/BOM)
...
/**
 * Finds the nearest commercial airport for a non-airport city based on lat/lng or city name.
 */
export function findNearestCommercialAirport(
  cityName: string,
  lat?: number,
  lng?: number
): { iata: string; name: string; city: string; distanceKm: number } {
  ...
  if (lat && lng) {
    ...
    let closest: CommercialAirport = COMMERCIAL_AIRPORTS[0];
