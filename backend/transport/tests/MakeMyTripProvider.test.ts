...
import { MakeMyTripProvider } from '../src/providers/MakeMyTripProvider';
import { StationResolver } from '../src/services/StationResolver';
import { CitySlugService } from '../src/services/CitySlugService';
import { MMTTrainScraper } from '../src/providers/makemytrip/MMTTrainScraper';
import { MMTBusScraper } from '../src/providers/makemytrip/MMTBusScraper';
...
describe('MakeMyTripProvider', () => {
          ...
          useValue: {
            resolve: jest.fn().mockImplementation((query: string) => {
              if (query === 'Goa' || query === 'MAO') {
                return Promise.resolve({ name: 'Madgaon Junction', code: 'MAO', city: 'Goa' });
              }
              return Promise.resolve({ name: 'Mumbai CSMT', code: 'CSTM', city: 'Mumbai' });
            }),
  ...
  describe('getTrainBookingUrl', () => {
    it('should generate official MMT train URL pattern', () => {
      ...
      expect(url).toBe('https://www.makemytrip.com/railways/listing.html?from=CSTM&to=MAO&departDate=2025-08-15&pax=1&class=3A');
    });
  });
  ...
  describe('getBusBookingUrl', () => {
    it('should generate official MMT bus URL pattern', () => {
      ...
      expect(url).toBe('https://www.makemytrip.com/bus-tickets/mumbai-to-pune/?dd=15&mm=08&yy=2026');
    });
  });
  ...
  describe('searchTrains', () => {
    it('should return SearchTrainsResult structure', async () => {
      const result = (await provider.searchTrains({
        ...
        passengers: 1,
        travelClass: '3A',
      })) as { provider: string; origin: { code: string }; searchUrl: string; results: unknown[] };
  ...
  describe('searchBuses', () => {
    it('should return SearchBusesResult structure', async () => {
      const result = (await provider.searchBuses({
        ...
        destinationCity: 'Pune',
        departureDate: '2026-08-15',
      })) as { provider: string; origin: { slug: string }; searchUrl: string; results: unknown[] };
      expect(result.provider).toBe('MakeMyTrip');
