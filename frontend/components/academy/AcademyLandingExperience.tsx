...
export default function AcademyLandingExperience({ onEnrolled }: AcademyLandingExperienceProps) {
  ...
  return (
    <div className="w-full relative text-[#1A1A1A] space-y-12 sm:space-y-24 pb-16 bg-[#FFFBF7]">
      {/* ─── 1. HERO SECTION (TripSage Warm Cream & 3D Bionic Singularity Hand) ─── */}
      <section className="relative w-full max-w-6xl mx-auto px-4 sm:px-6 pt-4 sm:pt-10 flex flex-col items-center text-center">
        {/* Top Academy Pill Badge */}
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="inline-flex items-center px-3.5 sm:px-4 py-1.5 rounded-full bg-orange-50 border border-orange-200 text-[#EA580C] text-[11px] sm:text-sm font-extrabold uppercase tracking-widest shadow-2xs mb-3 sm:mb-4"
        >
        ...
        <motion.h1
          ...
          className="text-3xl sm:text-5xl md:text-6xl lg:text-[64px] font-black tracking-tight text-[#1A1A1A] leading-[1.12] sm:leading-[1.1] max-w-4xl"
        ...
        {/* ─── 3D Bionic Hand Visual with Interactive Glass Cards ─── */}
        <div className="relative w-full max-w-4xl aspect-[4/3] sm:aspect-[16/9] mx-auto mt-5 sm:mt-6 rounded-2xl sm:rounded-3xl overflow-hidden shadow-[0_15px_45px_rgba(0,0,0,0.06)] border border-[#E8E0D8]/80 bg-white">
          <Image
          ...
          <motion.div
            ...
            className="hidden sm:flex absolute top-[28%] left-[6%] md:left-[8%] z-20 items-center gap-2.5 md:gap-3 px-3.5 md:px-4 py-2.5 rounded-2xl bg-white/90 backdrop-blur-md border border-[#E8E0D8] shadow-[0_8px_25px_rgba(0,0,0,0.08)] cursor-pointer hover:border-[#EA580C] hover:scale-105 transition-all group"
          >
            <div className="w-8 md:w-9 h-8 md:h-9 rounded-xl bg-orange-50 border border-orange-200 flex items-center justify-center text-[#EA580C] shadow-inner group-hover:bg-[#EA580C] group-hover:text-white transition-colors">
          ...
          {/* Desktop/Tablet Floating Card 2: Foundations of AGI (Left Bottom) */}
          <motion.div
            animate={{ y: [0, 5, 0] }}
            transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut', delay: 0.5 }}
            onClick={scrollToCurriculum}
            className="hidden sm:flex absolute bottom-[20%] left-[5%] md:left-[7%] z-20 items-center gap-2.5 md:gap-3 px-3.5 md:px-4 py-2.5 rounded-2xl bg-white/90 backdrop-blur-md border border-[#E8E0D8] shadow-[0_8px_25px_rgba(0,0,0,0.08)] cursor-pointer hover:border-[#EA580C] hover:scale-105 transition-all group"
          >
            <div className="w-8 md:w-9 h-8 md:h-9 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-600 shadow-inner group-hover:bg-amber-600 group-hover:text-white transition-colors">
              <Layers className="w-4 md:w-5 h-4 md:h-5" />
            </div>
            <div className="text-left">
              <div className="text-xs md:text-sm font-bold text-[#1A1A1A] tracking-tight">
                Foundations of AGI
          ...
          {/* Desktop/Tablet Floating Card 3: Agent Workflows (Right) */}
          <motion.div
            animate={{ y: [0, -6, 0] }}
            transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
            onClick={scrollToCurriculum}
            className="hidden sm:flex absolute top-[32%] right-[6%] md:right-[8%] z-20 items-center gap-2.5 md:gap-3 px-3.5 md:px-4 py-2.5 rounded-2xl bg-white/90 backdrop-blur-md border border-[#E8E0D8] shadow-[0_8px_25px_rgba(0,0,0,0.08)] cursor-pointer hover:border-[#EA580C] hover:scale-105 transition-all group"
          >
            <div className="w-8 md:w-9 h-8 md:h-9 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-blue-600 shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-colors">
              <Network className="w-4 md:w-5 h-4 md:h-5" />
            </div>
            <div className="text-left">
              <div className="text-xs md:text-sm font-bold text-[#1A1A1A] tracking-tight">
                Agent Workflows
        ...
        {/* Mobile Pillar Micro-Cards (Shown neatly below hero image on mobile) */}
        <div className="grid grid-cols-3 gap-2 mt-3 w-full max-w-4xl sm:hidden">
          <button
            onClick={scrollToCurriculum}
            className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white border border-[#E8E0D8] shadow-2xs active:scale-95 transition-all"
          >
            <div className="w-7 h-7 rounded-lg bg-orange-50 border border-orange-200 flex items-center justify-center text-[#EA580C] mb-1">
              <Brain className="w-3.5 h-3.5" />
            </div>
            <span className="text-[11px] font-bold text-[#1A1A1A] leading-tight">Reasoning</span>
            <span className="text-[9px] text-slate-500 font-medium">System 2</span>
          </button>
          ...
          <button
            ...
            className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white border border-[#E8E0D8] shadow-2xs active:scale-95 transition-all"
          >
            <div className="w-7 h-7 rounded-lg bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-600 mb-1">
          ...
          <button
            ...
            className="flex flex-col items-center text-center p-2.5 rounded-xl bg-white border border-[#E8E0D8] shadow-2xs active:scale-95 transition-all"
          >
            <div className="w-7 h-7 rounded-lg bg-blue-50 border border-blue-200 flex items-center justify-center text-blue-600 mb-1">
              <Network className="w-3.5 h-3.5" />
            </div>
            <span className="text-[11px] font-bold text-[#1A1A1A] leading-tight">Agents</span>
            <span className="text-[9px] text-slate-500 font-medium">Workflows</span>
          </button>
        </div>
      ...
      <section id="about" className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
        <div className="p-5 sm:p-10 rounded-2xl sm:rounded-3xl bg-white border border-[#E8E0D8] shadow-2xs space-y-3 sm:space-y-4 text-center">
          ...
          <p className="text-xs sm:text-sm text-[#57534E] leading-relaxed max-w-2xl mx-auto">
      ...
      <section className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
        <div className="p-6 sm:p-12 rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#FFF8F3] to-[#FFFBF7] border border-orange-200 text-center space-y-5 sm:space-y-6 shadow-md relative overflow-hidden">
          <div className="space-y-2 relative z-10">
            <h3
              className="text-xl sm:text-3xl lg:text-4xl font-black text-[#1A1A1A] tracking-tight"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              Start the Foundations of AGI Today
            </h3>
            <p className="text-xs sm:text-base text-[#57534E] max-w-lg mx-auto">
          ...
          <div className="flex justify-center relative z-10 w-full">
            <button
              onClick={() => setModalOpen(true)}
              className="w-full sm:w-auto px-10 py-3.5 sm:py-4 rounded-full bg-[#EA580C] hover:bg-[#C2410C] text-white font-bold text-sm sm:text-base shadow-[0_8px_25px_rgba(234,88,12,0.35)] active:scale-98 transition-all"
            >
              Enroll for Free →
            </button>
          </div>
