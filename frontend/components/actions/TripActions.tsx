...
export default function TripActions() {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `Check out my AI-planned trip to ${tripContext.destination}!`,
      url: shareUrl,
    }
  ...
  const handleDownload = () => {
    ...
    const flightSection = bookedFlight ? `
      <div class="booking-box">
        <span class="badge">FLIGHT BOOKED</span>
        <p><strong>${bookedFlight.name}</strong></p>
        <p>Departure: ${bookedFlight.departure} → ${bookedFlight.arrival} &nbsp;|&nbsp; Duration: ${bookedFlight.duration}</p>
        <p>Price: ${formatPrice(bookedFlight.price, currency)} per person</p>
    ...
    const hotelSection = bookedHotel ? `
      <div class="booking-box">
        ...
        <p>Location: ${bookedHotel.location} &nbsp;|&nbsp; Rating: ${bookedHotel.rating}/5</p>
        <p>Price: ${formatPrice(bookedHotel.price, currency)}/night</p>
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        ...
        ) : <Download size={18} className="transition-all hover:brightness-125 duration-300" />}
      ...
      >
        {sharing ? (
          <span className="w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin" />
        ) : <Share2 size={18} className="transition-all hover:brightness-125 duration-300" />}
