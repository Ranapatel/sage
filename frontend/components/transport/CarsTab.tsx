...
function CarsTab() {
  ...
  const redirectUrl = `https://www.discovercars.com/?a_aid=tripsage`

  // If store has real car objects, render CarCard grid
  if (cars && Array.isArray(cars) && cars.length > 0 && (cars[0] as any).carType) {
    return (
      <div className="space-y-4 animate-fade-in">
        <div className="text-xs font-bold text-slate-500 mb-1">
          {cars.length} cab & car options available
        </div>
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 hover:border-orange-500/50 hover:shadow-lg transition-all duration-300 p-4 md:p-5 flex flex-col md:flex-row items-center justify-between gap-4 group">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-600 flex-wrap justify-center">
          ...
          <span className="bg-[#FFF7ED] text-[#EA580C] font-bold px-3 py-1 rounded-full border border-orange-200">
            No Hidden Fees
          </span>
        </div>
