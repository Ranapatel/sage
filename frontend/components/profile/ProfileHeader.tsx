...
export default function ProfileHeader({ user, stats }: ProfileHeaderProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-[#E8E0D8] bg-white shadow-sm">
      {/* Subtle background glows — desktop only to save paint */}
      <div className="hidden md:block absolute -top-24 -left-24 h-48 w-48 rounded-full bg-orange-500/5 blur-3xl" />
      <div className="hidden md:block absolute -bottom-24 -right-24 h-48 w-48 rounded-full bg-amber-500/5 blur-3xl" />

      {/* ── MOBILE: compact single-row strip ──────────────────────────── */}
      <div className="flex md:hidden items-center gap-3 px-4 py-3 relative z-10">
        {/* Avatar */}
        <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/heic" onChange={handleAvatarUpload} className="hidden" />
        <div
          onClick={() => fileInputRef.current?.click()}
          className="relative h-10 w-10 shrink-0 overflow-hidden rounded-xl border border-[#E8E0D8] shadow-sm group cursor-pointer"
          title="Change photo"
        >
          {profileImage ? (
            <Image src={profileImage} alt={fullName} fill sizes="40px" className="object-cover" unoptimized />
          ) : (
            <div className="flex h-full w-full items-center justify-center bg-[#FFFBF7] text-base font-black text-[#EA580C]">
              {fullName.charAt(0)}
            </div>
          )}
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-black text-[#1A1A1A] truncate leading-tight">{fullName}</p>
          <p className="text-[11px] text-slate-400 truncate">{user.email}</p>
        </div>

        {/* Tier badge */}
        <span className="shrink-0 text-[10px] font-bold text-[#EA580C] bg-orange-50 border border-orange-200 rounded-full px-2 py-0.5">
          {tierLabel}
        </span>
      </div>

      {/* ── DESKTOP: full layout with stats ──────────────────────────── */}
      <div className="hidden md:block p-6 md:p-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
            ...
            >
              {profileImage ? (
                <Image src={profileImage} alt={fullName} fill sizes="80px" className="object-cover" unoptimized />
              ) : (
              ...
              <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity">
                {uploading ? <Loader2 size={20} className="animate-spin text-white" /> : <><Camera size={18} /><span className="text-[9px] font-bold mt-0.5">Upload</span></>}
              </div>
            </div>
            <div>
